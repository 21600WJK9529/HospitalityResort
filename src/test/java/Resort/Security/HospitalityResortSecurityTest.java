...
public class HospitalityResortSecurityTest {
    ...
    @Test
    public void update(){
        MaintenanceRegister maintenanceRegister = MaintenanceFactory.getMaintenanceRegister("id","newFName565", "newLName2","newEmail2","newFacility","newPhoneNo");
    ...
    public void delete() {
        ResponseEntity result = restTemplate.withBasicAuth("admin", "password")
                .getForEntity(BASE_URL+"/delete/phpid", String.class);
        System.out.println(result.getBody());
        assertEquals(HttpStatus.OK, result.getStatusCode());
    }
}
